...
- superSamuraiSlice：`bop`/小恶魔/大恶魔/平台滚动/环境开关的事件流、输入、
  ...
  仍待把 Unity ParticleSystem 序列化参数通用导出后逐项复刻。
...
- airboarder：`bop`/`duck`/`crouch`/`jump`/`forceCharge`/`letsGo`/背景地板
  ...
  FBX Geometry 已导出，`assets/airboarder/meshes.json` 已入库并由运行时加载/
  绘制单 Geometry MeshRenderer。原版主体是 MeshRenderer/材质贴图的 3D 场景，
  当前仍暂用手写 2D billboard 渲染角色/障碍；运行时仍待补多 Geometry
  FBX 映射、材质贴图采样、CameraPivot/FOV 与
