...
- karateman：Pack-In 使用路径已接入 engine（hit/bop/prepare/warnings/background/
  ...
  bomb/kick 在命中后、NG 与飞行超时清理阶段的延迟爆点也已按
  `KarateManPot.Update` 分支补齐。
...
- chargingChicken：`input`/`journeyLength`/倒计时泡泡/文本编辑/音乐淡入淡出/
  ...
  `particles.json` burst 参数接入；StonePlatform 逐实例 Set/Fall/Splash、
  block/car 落水音效和成功复位 offset 已按 `Island.cs` 时序接入。
