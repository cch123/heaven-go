...
**已移植游戏**：rhythmSomen（Rhythm Sōmen）、trickClass（Trick on the Class）、karateman（旧 demo 路径）。
...
go run .                                 # 标题屏，把 .riq 拖进窗口即可游玩
go run . -riq "<path>.riq"               # 或命令行指定
```

操作：`Space` / `J` / 鼠标左键；`Tab` 调试叠层；`R` 结算后重开；`Esc` 退出。可随时把新的 `.riq` 拖入窗口切换关卡。
...
| `kart` | 运行时：图集子图、仿射骨架/场景合成、曲线采样（Hermite + 阶跃换帧 + FlipX + m_IsActive 层级传播 + m_Color）；`SceneInst` 支持多 Animator 并行，剪辑时间 = 拍数 × timeScale | Animator / SpriteRenderer / DoScaledAnimationAsync |
...
- 未实现：音量变化、section 标记、swing、运行时变调（pitch change）、`karateman/hit` 的 `type` 参数（石头/灯泡等变体）、AnimatorController 状态机（直接按名播放 clip）、表情/特殊镜头、输入延迟校准。
